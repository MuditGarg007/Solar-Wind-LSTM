"""Phase M Step 5-6: CME+flare+F10.7 INPUTS.
5-seed paired A/B: base(29) vs cme(37=+8 solar-source feats), identical windows/splits.
Leakage-safe: solar-source feats causal-lagged, input-window only. Held-out test eval.
Focus t+5/t+6 (CME transit-lead hypothesis). Writes cme_ab.csv. Mirrors _ab_train_indices.py."""
...
b=np.load('_seq_cme_base.npz'); a=np.load('_seq_cme.npz')
Ytr=b['Ytr']; Yv=b['Yv']; Yte=b['Yte']
assert np.array_equal(b['Yte'],a['Yte']) and np.array_equal(b['Ytr'],a['Ytr'])  # paired
...
for seed in SEEDS:
    ...
    pd.DataFrame(all_rows).to_csv('cme_ab.csv',index=False)

df=pd.DataFrame(all_rows)
print('\n=== PAIRED Δ (base - cme); +ve = cme better ===')
for hor in ['all','t+1','t+4','t+5','t+6']:
    for reg in (['agg','intense','moderate','quiet'] if hor=='all' else ['agg','intense']):
        ...
        dd=pv['base']-pv['cme']
        print(f'{hor:4s} {reg:9s}: base {pv["base"].mean():.2f}  cme {pv["cme"].mean():.2f}  '
              f'Δ {dd.mean():+.2f}±{dd.std():.2f}  (seeds +: {int((dd>0).sum())}/{len(dd)})')
print('\nsaved cme_ab.csv')
